...
(function () {
    ...
    function loginUserInternal(tenantId, callback) {
            ...
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
                    ...
                    if (responseJSON.success) {
                        ...
                        abp.utils.setCookieValue('enc_auth_token', result.encryptedAccessToken, expireDate, abp.appPath)
                        callback();
                    } else {
                        alert("Login failed !");
                    }
        ...
        if (window.location.href.indexOf("1.15.55.23:90") >= 0) {
            xhr.open('POST', '/apis/user/login', true)
        } else {
            xhr.open('POST', '/user/login', true);
        }
        xhr.setRequestHeader('Abp.TenantId', tenantId);
        xhr.setRequestHeader('Content-type', 'application/json');
        xhr.send("{" + "\"email\":\"" + usernameOrEmailAddress + "\",\"password\":\"" + password + "\"}");
    };

    abp.swagger.login = function (callback) {
        //Get TenantId first
        var tenancyName = null;// document.getElementById('tenancyName').value;
    ...
    abp.swagger.logout = function () {
        abp.auth.clearToken();
        abp.utils.deleteCookie('enc_auth_token', abp.appPath);
    }
